...
app.post("/generate", async (req, res) => {
    ...
    try {
        ...
        const contentOffsetY = (1920 - 1080) / 2;

        /* ================= BACKGROUND ================= */

        const bg = await loadImage("./paper_background.png");
        ctx.drawImage(bg, 0, contentOffsetY, 1080, 1080);
        ...
        const profileX = 90;
        const profileY = contentOffsetY + TOP_PADDING;
        ...
        const imageY = contentOffsetY + 1080 - finalSize - BOTTOM_MARGIN;
        const imageX = (canvas.width - finalSize) / 2;

        ctx.save();
        ctx.filter = "grayscale(100%) contrast(100%) brightness(90%)";
        ...
        const tempImage = path.join(__dirname, "temp.png");
        const tempVideo = path.join(__dirname, "final_video.mp4");

        fs.writeFileSync(tempImage, canvas.toBuffer("image/png"));
        ...
        ffmpeg()
            ...
            .outputOptions([
                "-vf scale=1080:1920",
                "-af volume=1.5",   // 🔊 increase music volume
                "-pix_fmt yuv420p",
                "-r 30",
                "-shortest"
            ])
            ...
            .on("end", () => {
                ...
                res.set({
                    ...
                    "Content-Disposition": "attachment; filename=quote_video.mp4"
                });

                const stream = fs.createReadStream(tempVideo);
                stream.pipe(res);
...
app.listen(3001, () => {
    console.log("🔥 Reels Video Engine Running at http://localhost:3001");
});